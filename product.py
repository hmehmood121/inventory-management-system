...
class productClass:
    def __init__(self,root):
        ...
        self.root.title("Inventory Management System | Developed by Hasan Mehmood")
...
# ====================column 2==============================
        ...
        lbl_mfg=Label(product_Frame,text="Mfg",font=("goudy old style",18),bg="white").place(x=40,y=360)
        lbl_exp=Label(product_Frame,text="Exp",font=("goudy old style",18),bg="white").place(x=260,y=360)
        ...
        txt_mfg=Entry(product_Frame,textvariable=self.var_mfg,font=("goudy old style",15),bg="lightyellow").place(x=335,y=365,width=100)
        txt_exp=Entry(product_Frame,textvariable=self.var_exp,font=("goudy old style",15),bg="lightyellow").place(x=130,y=365,width=100)

        # =============================Buttons=============================

        btn_add=Button(product_Frame,text="Save",command=self.add,font=("goudy old style",15),bg="#2196f3",fg="white",cursor="hand2").place(x=10,y=420,width=100,height=40)
        btn_update=Button(product_Frame,text="Update",command=self.update,font=("goudy old style",15),bg="#4caf50",fg="white",cursor="hand2").place(x=120,y=420,width=100,height=40)
        btn_delete=Button(product_Frame,text="Delete",command=self.delete,font=("goudy old style",15),bg="red",fg="white",cursor="hand2").place(x=230,y=420,width=100,height=40)
        btn_clear=Button(product_Frame,text="Clear",command=self.clear,font=("goudy old style",15),bg="#607d8b",fg="white",cursor="hand2").place(x=340,y=420,width=100,height=40)
   ...
   # ============Employee Details====================
        ...
        self.product_table=ttk.Treeview(emp_frame, columns=("pid","Supplier","Category","name","price","qty","status","mfg","exp"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        ...
        self.product_table.heading("mfg",text="Mfg")
        self.product_table.heading("exp",text="Expiry")
        ...
        self.product_table.column("mfg",width=100)
    ...
    def add(self):
        ...
        try:
            if self.var_cat.get()=="Select" or self.var_cat.get()=="Empty" or self.var_sup.get=="Select" or self.var_name=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error", "Product already exist. Try diiferent.",parent=self.root)
                else:
                    cur.execute("Insert into product (Category,Supplier,name,price,qty,status,mfg,exp) values(?,?,?,?,?,?,?,?)",(
                    self.var_cat.get(),
                    self.var_sup.get(),
                    self.var_name.get(),
                    self.var_price.get(),
...
####################### Show Function ###################
 ...
 ############### Expiry Date Function ################
    ...
    #     cur=con.cursor()
    #     try:
            # cur.execute("Select * from product where exp < DATE('now')")
            # rows=cur.fetchall()
    #         for row in rows:
    #             self.product_table('',END,values=row)
    #     except Exception as ex:
    #         messagebox.showerror("Error",f"Error due to : {str(ex)}",parent=self.root)
        ...
        try:
            if self.var_pid.get()=="":
                ...
                if row==None:
                    ...
                    cur.execute("Update product set Category=?,Supplier=?,name=?,price=?,qty=?,status=?,mfg=?,exp=? where pid=?",(
                    self.var_cat.get(),
                    self.var_sup.get(),
                    self.var_name.get(),
                    self.var_price.get(),
